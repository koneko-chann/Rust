...
pub fn verify_jwt_token<T:FromStr>(token: &str) -> Result<T, jwt_simple::Error> {
    ...
    let user_id_str = claims.subject.ok_or(anyhow!("Missing subject in token claims"))?;
    let user_id = user_id_str.parse::<T>().map_err(|_| anyhow!("Invalid user ID in token claims"))?;
    Ok(user_id)
}
...
mod tests {
    ...
    fn test_jwt_token_creation_and_verification() {
        ...
        let verified_user_id = verify_jwt_token::<String>(&token).expect("Token verification failed");
    ...
    fn test_password_hashing_and_verification() {
        ...
        let is_invalid = verify_password("wrong_password", &hashed).expect("Password verification failed");
