...
pub async fn mw_auth_with_secret_key(req: Request, next: Next) -> AppResult<Response> {
    ...
    let header_key = req
        ...
        .map(|s| s.trim())
        .map(|s| s.strip_prefix("Bearer").unwrap_or_default().trim());
...
pub async fn mw_auth_with_jwt(mut req: Request, next: Next) -> AppResult<Response> {
    ...
    match is_valid {
        Ok(_user_id) => {
            ...
            req.extensions_mut().insert(AuthUser {
                user_id: _user_id.parse::<i64>().unwrap_or_default(),
            });
            Ok(next.run(req).await)
        }
        Err(_) => {
